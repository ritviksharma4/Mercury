...
def convert_input_to_tensor_dataset(lines,
                                         pred_config,
                                         args,
                                         tokenizer,
                                         pad_token_label_id,
...
def predict(pred_config):
    ...
    lines = read_input(pred_config)
    dataset = convert_input_to_tensor_dataset(lines, pred_config, args, tokenizer, pad_token_label_id)
    ...
    for words, slot_preds, intent_pred in zip(lines, slot_preds_list, intent_preds):
        ...
        for word, pred in zip(words, slot_preds):
            ...
            if pred != 'O':
                ...
                elif intent_label_lst[intent_pred] == "OrderFood":
                    for _ in range (0,index+2):
                        next_of_qty = next(iter_slot_preds)
                    if next_of_qty != "B-food_type":
                        collected_slots.setdefault('food_type', []).append('')
